...
public enum ApiErrorCode {
    ...
    RESET_PASSWORD_TOKEN_EXPIRED(10503, "Reset password token expired", "Given reset password token is expired", HttpStatus.BAD_REQUEST),
    RESET_PASSWORD_TOKEN_NOT_FOUND(10501, "Reset password token not found", "Reset password token not found", HttpStatus.NOT_FOUND),
    RESET_PASSWORD_TOKEN_MISMATCH(10507, "Reset password token mismatch", "Given reset password token doesn't match with requested account", HttpStatus.CONFLICT),
    ...
    REFRESH_TOKEN_EXPIRED(10507, "Refresh token expired", "Refresh token is expired", HttpStatus.CONFLICT),
    ...
    INVALID_JWT_TOKEN(10507, "Invalid JWT token", "An error occurs during parsing a JWT token", HttpStatus.INTERNAL_SERVER_ERROR),
