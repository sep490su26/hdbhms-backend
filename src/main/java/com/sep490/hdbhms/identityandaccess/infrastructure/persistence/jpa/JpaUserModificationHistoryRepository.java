...
public interface JpaUserModificationHistoryRepository extends JpaRepository<UserModificationHistoryEntity, String> {
    ...
    @Query(value = """
                SELECT changed_at FROM hdbhms.user_modification_histories WHERE user_id = ?1 AND type = 'USERNAME' ORDER BY changed_at DESC LIMIT 1
            """, nativeQuery = true)
    Instant getLatestUsernameModificationTimestamp(Long accountId);

    @Query(value = """
                SELECT changed_at FROM hdbhms.user_modification_histories WHERE user_id = ?1 AND type = 'EMAIL' ORDER BY changed_at DESC LIMIT 1
            """, nativeQuery = true)
    Instant getLatestEmailModificationTimestamp(Long accountId);

    @Query(value = """
                SELECT old_value FROM hdbhms.user_modification_histories WHERE user_id = ?1 AND type = 'PASSWORD_CHANGE' ORDER BY changed_at DESC LIMIT ?2
            """, nativeQuery = true)
    Set<String> getTopLatestPasswordChangeValuesByAccount_Id(Long accountId, int top);

    @Query(value = """
                SELECT changed_at FROM hdbhms.user_modification_histories WHERE user_id = ?1 AND type = 'PASSWORD_RESET' ORDER BY changed_at DESC LIMIT 1
            """, nativeQuery = true)
